...
public abstract class BaseGame extends SurfaceView implements SurfaceHolder.Callback, Runnable{
    ...
    public BaseGame(Context context, Handler handler, boolean hasMusic, boolean isOnline){
        ...
        //播放bgm
        mysp = new MySoundPool(context, hasMusic);
        bgm = new MyMediaPlayer(context, R.raw.bgm,true,hasMusic);
        ...
        if (enemyCounter >= enemyCycle) {
            ...
            if (enemyAircrafts.size() < enemyMaxNumber) {
                if (random.nextDouble() < eliteProb) {
                    //精英机
                    AbstractEnemyAircraft newElite = eliteEnemyFactory.createEnemyAircraft(gameLevel);
                    res.add(newElite);
                    BombSupply.addFlyingObject(newElite);
                }
                else{
                    //普通敌机
                    AbstractEnemyAircraft newMob = mobEnemyFactory.createEnemyAircraft(gameLevel);
                    res.add(newMob);
                    BombSupply.addFlyingObject(newMob);
                }
    ...
    private void crashCheckAction() throws InterruptedException {
        ...
        for (AbstractFlyingSupply flyingSupply : flyingSupplies) {
            ...
            if (heroAircraft.crash(flyingSupply) || flyingSupply.crash(heroAircraft)) {
                ...
                score += flyingSupply.getAddScore();
                flyingSupply.vanish();
            }
        }
    }
